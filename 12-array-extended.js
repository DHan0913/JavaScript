...
function testFilter() {
    console.log("===== filter");
     ...
     for (let i =0; i < numbers.length; i++){
        if(numbers[i] % 2 == 0) {
            multiply.push(numbers[i]);
        } 
     }
     console.log("기존방식: 짝수", multiply);
...
function testReduce3() {
    ...
    //  reduce 함수를 이용, numbers 배열의 짝수 함수 배열을 만들어보기
    console.log("원본배열:", numbers);
...
function testDataPipeline() {
    ...
    const sortedStudents2 = filteredStudents.sort((a, b)=> b.total - a.total);   //  DESC 
    console.log("total순 ASC:", sortedStudents);
    console.log("total순 DESC:", sortedStudents2);
    ...
    console.log("총점 240점 이상 학생들의 총점 평균:", avgTotalScore);
